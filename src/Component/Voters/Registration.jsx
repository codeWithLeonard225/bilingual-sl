...
const Registration = () => {
    ...
    // 🌟 NEW EFFECT: Real-time Listener for Classes (Classes collection)
    useEffect(() => {
        const classRef = collection(db, "Classes");
        const q = query(classRef);

        const unsubscribe = onSnapshot(q, (snapshot) => {
            // Map the documents to an array of class names
            const options = snapshot.docs.map(doc => doc.data().className).sort();
    ...
    const filteredUsers = useMemo(() => {
        ...
        return users.filter(user => {
            ...
            // Match if EITHER the name OR the class contains the search term.
